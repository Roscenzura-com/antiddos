...
1. Добавьте в самый верх исполняемого файла (как правило index.php) код `include($_SERVER['DOCUMENT_ROOT'].'/<папка скрипта>/include.php');`
2. Если ддосят только главную, поменяйте в файле `<папка скрипта>/include.php` переменную $url на `$url='/';`

**Важно!**	При работе с Cloudflare можно поставить исключения для кешируемых файлов, если они загружаются через скрипт. Все равно ддосер напрямую их атаковать не сможет. 
...
{
	include($_SERVER['DOCUMENT_ROOT'].'/antiddos333/include.php');
}
```
